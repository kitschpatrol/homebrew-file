...
def test_clear_input(brew_info):
    ...
    brew_info.cask_args_input.extend(['abc', 'efg'])
...
def test_sort(brew_info):
    ...
    brew_info.tap_list.extend(['rcmdnk/file', 'homebrew/cask', 'homebrew/bundle', 'homebrew/core'])
    brew_info.appstore_list.extend(['111 ccc (2)', '222 aaa (1)', 'bbb'])
    brew_info.sort()
    assert brew_info.tap_list == ['homebrew/core', 'homebrew/cask', 'homebrew/bundle', 'rcmdnk/file']
...
def test_add(brew_info):
    ...
    assert brew_info.brew_input_opt == {'aaa': 'ddd', 'bbb': 'bbb', 'ccc': 'ccc'}


def test_read(brew_info):
    brew_info.read()
    assert brew_info.brew_input_opt == {'vim': ' --HEAD', 'python': ''}
    assert brew_info.brew_input == ['vim', 'python']
    assert brew_info.tap_input == ['direct', 'homebrew/cask', 'rcmdnk/rcmdnkcask']
    ...
    assert brew_info.file_input == ['BrewfileMain', 'BrewfileExt', 'BrewfileExt2', 'BrewfileNotExist', '~/BrewfileHomeForTestingNotExists']
...
def test_mas_pack(brew_info):
    brew_info.helper.opt['form'] = 'file'
    assert brew_info.mas_pack('409183694   Keynote  (12.2.1)') == '409183694   Keynote  (12.2.1)'
    brew_info.helper.opt['form'] = 'bundle'
    assert brew_info.mas_pack('409183694   Keynote  (12.2.1)') == "'Keynote (12.2.1)', id: 409183694"


@pytest.mark.filterwarnings("ignore::DeprecationWarning")
def test_write(brew_info):
    default_file = brew_info.filename
    with NamedTemporaryFile() as f:
        ...
        brew_info.read()
        brew_info.input_to_list()
        brew_info.filename = Path(f.name)
        brew_info.write()
        ...
        with open(f.name) as f2:
            assert f2.read() == """# Before commands
#before echo before
...
"""
        ...
        brew_info.write()
        with open(f.name) as f2:
            assert f2.read() == """#!/usr/bin/env bash
